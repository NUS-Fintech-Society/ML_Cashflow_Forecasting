...
"""Inputs"""
customer = 83 # input customerID here
...
# Model features
features=['customerID', 'InvoiceDate', 'DueDate', 'InvoiceAmount', 'repeatCust', 'Disputed', 'Late']
...
due_date = dt.datetime.toordinal(due_date)
invoice_date = datetime.strptime(invoice_date, '%Y/%m/%d').date()
invoice_date = dt.datetime.toordinal(invoice_date)
late = np.float64(late) / 100


# Create new dataframe based on inputs
input_variables = pd.DataFrame([[customer, invoice_date, due_date, invoice_amount, num_of_past_invoices, disputed, late]], dtype=float)

# Get the model's prediction
prediction = GBR_model.predict(input_variables).round(0).astype(int)
prediction = int(prediction)
predicted_date = datetime.fromordinal(due_date).date()
